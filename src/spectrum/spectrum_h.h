...
fft_real W(fft_real k1, fft_real k2, fft_real k3, fft_real Lb, fft_real R,
           int window);

/*Define the bin for the mode*/
int Indice(fft_real k, fft_real kmin, fft_real dk);

/*Compute the density grids in Fourier space corrected by the interlacing and
 * window function*/
void Compute_gridk(fft_real *grid, fft_complex **gridk, int nd, fft_real L,
                   int ntype, int interlacing, fft_real R, int window);
