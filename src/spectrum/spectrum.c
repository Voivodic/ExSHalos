...
static PyMethodDef spectrum_methods[] = {
    {"check_precision", (PyCFunction)spectrum_check_precision, METH_VARARGS, "Returns precision used by the estimators of the spectra"},
    {"grid_compute", (PyCFunction)grid_compute, METH_VARARGS | METH_KEYWORDS, "Computes the density grid of a given list of particles"},
    {"power_compute", (PyCFunction)power_compute, METH_VARARGS | METH_KEYWORDS, "Computes the PowerSpectrum of a given density grid"},
    {"power_compute_individual", (PyCFunction)power_compute_individual, METH_VARARGS | METH_KEYWORDS, "Computes the PowerSpectrum of a given density grid for individual particles"},
    {"bi_compute", (PyCFunction)bi_compute, METH_VARARGS | METH_KEYWORDS, "Computes the BiSpectrum of a given density grid"},
    {"tri_compute", (PyCFunction)tri_compute, METH_VARARGS | METH_KEYWORDS, "Computes the TriSpectrum of a given density grid"},
    {"abundance_compute", (PyCFunction)abundance_compute, METH_VARARGS | METH_KEYWORDS, "Computes the differential abundance of halos"},
    {"histogram_compute", (PyCFunction)histogram_compute, METH_VARARGS | METH_KEYWORDS, "Computed the (un)masked histogram of delta"},
    {NULL, NULL, 0, NULL}
};
...
static PyObject *grid_compute(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    if(direction != -1)
        vel = (fft_real *) PyArray_DATA(vel_array);
    else
        vel = NULL;
	if(nmass == TRUE)
		mass = (fft_real *) PyArray_DATA(mass_array);
	else
		mass = NULL;
	if(ntype > 1)
		type = (int *) PyArray_DATA(type_array);
	else
		type = NULL;
	...
	grid = (fft_real *) PyArray_DATA(np_grid);
...
static PyObject *power_compute(PyObject *self, PyObject *args, PyObject *kwargs){
	...
	/*Convert the PyObjects to C arrays*/
	grid = (fft_real *) PyArray_DATA(grid_array);
    NPs = (ntype*(ntype+1))/2;
    ls = floor(l_max/2) + 1;
    ...
    P_out = (fft_real *) PyArray_DATA(np_P);
    Kmean_out = (fft_real *) PyArray_DATA(np_k);
    count_k = (long *) PyArray_DATA(np_count_k);
...
static PyObject *power_compute_individual(PyObject *self, PyObject *args, PyObject *kwargs){
	...
	grid = (fft_real *) PyArray_DATA(grid_array);
    pos = (fft_real *) PyArray_DATA(pos_array);
    np = (int) PyArray_DIMS(pos_array)[0];
    NPs = ntype*np;
    ls = floor(l_max/2) + 1;
...
static PyObject *bi_compute(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    P_out = (fft_real *) PyArray_DATA(np_P);
    KP_out = (fft_real *) PyArray_DATA(np_kP);
    count_k = (fft_real *) PyArray_DATA(np_count_k);
    B_out = (fft_real *) PyArray_DATA(np_B);
    KB_out = (fft_real *) PyArray_DATA(np_kB);
    count_tri = (fft_real *) PyArray_DATA(np_count_tri);
...
static PyObject *tri_compute(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    KP_out = (fft_real *) PyArray_DATA(np_kP);
    count_k = (fft_real *) PyArray_DATA(np_count_k);
    T_out = (fft_real *) PyArray_DATA(np_T);
    Tu_out = (fft_real *) PyArray_DATA(np_Tu);
    KT_out = (fft_real *) PyArray_DATA(np_kT);
    count_sq = (fft_real *) PyArray_DATA(np_count_sq);
...
static PyObject *abundance_compute(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    Mmean = (fft_real *) PyArray_DATA(np_Mmean);
    dn = (fft_real *) PyArray_DATA(np_dn);
    dn_err = (fft_real *) PyArray_DATA(np_dn_err);
...
static PyObject *histogram_compute(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    /*Convert the PyObjects to C arrays*/
    nh = (size_t) PyArray_DIMS(Mh_array)[0];
    ndx = (size_t) PyArray_DIMS(delta_array)[0];
    ndy = (size_t) PyArray_DIMS(delta_array)[1];
    ndz = (size_t) PyArray_DIMS(delta_array)[2];
    ng = ndx*ndy*ndz;
    delta = (fft_real *) PyArray_DATA(delta_array);
    Mh = (fft_real *) PyArray_DATA(Mh_array);
    ...
    hist_unmasked = (long *) PyArray_DATA(np_hist_unmasked);
    hist_masked = (long *) PyArray_DATA(np_hist_masked);
    Mmean = (fft_real *) PyArray_DATA(np_Mmean);
    delta_mean = (fft_real *) PyArray_DATA(np_delta_mean);
