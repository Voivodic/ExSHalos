...
def Generate_Halos_Box_from_Pk(k, P, R_max = 100000.0, nd = 256, ndx = None, ndy = None, ndz = None, Lc = 2.0, Om0 = 0.31, z = 0.0, k_smooth = 10000.0, delta_c = -1.0, Nmin = 1, a = 1.0, beta = 0.0, alpha = 0.0, seed = None, fixed = False, phase = 0.0, OUT_DEN = False, OUT_LPT = False, OUT_VEL = False, OUT_PROF = False, DO_2LPT = False, OUT_FLAG = False, verbose = False, nthreads = 1):
    """
    ...
    x = exshalos.exshalos.exshalos.halos_box_from_pk(k, P, R_max, np.int32(ndx), np.int32(ndy), np.int32(ndz), Lc, np.int32(seed), k_smooth, Om0, z, delta_c, np.int32(Nmin), a, beta, alpha, np.int32(fixed), phase, np.int32(OUT_DEN), np.int32(OUT_LPT), np.int32(OUT_VEL), np.int32(DO_2LPT), np.int32(OUT_FLAG), np.int32(OUT_PROF), np.int32(verbose), np.int32(nthreads))

    return x

#Generate a halo catalogue from a density grid
def Generate_Halos_Box_from_Grid(grid, k, P, S = None, V = None, Lc = 2.0, Om0 = 0.31, z = 0.0, k_smooth = 10000.0, delta_c = -1.0, Nmin = 1, a = 1.0, beta = 0.0, alpha = 0.0, OUT_LPT = False, OUT_VEL = False, DO_2LPT = False, OUT_FLAG = False, OUT_PROF = False, verbose = False, nthreads = 1):
    """
    ...
    x = exshalos.exshalos.exshalos.halos_box_from_grid(k, P, grid, S, V, Lc, k_smooth, Om0, z, delta_c, np.int32(Nmin), a, beta, alpha, np.int32(OUT_LPT), np.int32(OUT_VEL), np.int32(DO_2LPT), np.int32(OUT_FLAG), np.int32(In_disp), np.int32(OUT_PROF), np.int32(verbose), np.int32(nthreads))
