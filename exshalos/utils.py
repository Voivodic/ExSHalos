...
def Find_Halos_from_Grid(grid, k, P, Lc = 2.0, Om0 = 0.31, z = 0.0, delta_c = -1.0, Nmin = 10, a = 1.0, beta = 0.0, alpha = 0.0, OUT_FLAG = False, OUT_PROF = False, verbose = False):
    """
    ...
    x = exshalos.exshalos.exshalos.find_halos(grid, k, P, Lc, Om0, z, delta_c, np.int32(Nmin), a, beta, alpha, np.int32(OUT_FLAG), np.int32(OUT_PROF), verbose)
...
def Compute_Correlation(k, P, direction = 1, verbose = False):
    ...
    x = exshalos.analytical.analytical.correlation_compute(k, P, np.int32(direction), np.int32(verbose))
