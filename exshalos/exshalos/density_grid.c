...
void Compute_Sig(int Nr, fft_real *R, fft_real *M, fft_real *Sig, fft_real *Sig_grid, fft_real *K, fft_real *P, int Nk){
    ...
    for(i=0;i<Nr;i++){
        ...
        M[i] = 4.0/3.0*M_PI*((fft_real)cosmo.rhomz*pow(R[i], 3));
    }
    ...
    gsl_spline_init(spline, (double *) M, (double *) Sig, Nr);
...
void Compute_nh(int model, int Nr, fft_real *R, fft_real *M, fft_real *Sig, gsl_spline *spline_I, gsl_spline *spline_InvI){
    ...
    /*Alloc the arrays*/
    Int = (fft_real *)malloc(Nr*sizeof(fft_real));
    check_memory(Int, "Int")
    Int[0] = 0.0;

    /*Compute the mass function*/
    for(i=1;i<Nr;i++)
        Int[i] = Int[i-1] - (log(Sig[i]) - log(Sig[i-1]))/2.0*(fh(Sig[i], model)/pow(R[i], 3.0) + fh(Sig[i-1], model)/pow(R[i-1], 3.0));

    /*Interpolate the integral of the mass function as function of mass and its inverse*/
    gsl_spline_init(spline_I, (double *) M, (double *) Int, Nr);
    gsl_spline_init(spline_InvI, (double *) Int, (double *) M, Nr);
