...
void Tracer_Grid(fft_real *grid, int nd, fft_real L, int direction, fft_real *pos, fft_real *vel, size_t np, fft_real *mass,  int *type, int ntype, int window, fft_real R, fft_real R_times, int interlacing){
	...
	/*Case with multiple types without weight (mass) between the particles and with interlacing*/
			...
			if(ntype == 1){
				...
				for(j=0;j<np;j++){
					...
					for(i=0;i<3;i++)
						post[i] = cysumf(post[i], -Lb/2.0, L);
					ind = ((size_t) ntype)*ng;
					M2_private[type[j]] += Density_Grid(&grid[((size_t) type[j])*ng + ind], nd, L, post, 1.0, window, R, R_times);
				}			
			}
		...
		for(i=0;i<ntype;i++){
			ind = ((size_t) i)*ng;
			ind2 = ((size_t) ntype)*ng;
			#pragma omp parallel for private(j) shared(ind, ind2, grid, M1, M2, ng)
			for(j=0;j<ng;j++){
				grid[ind + j] = grid[ind + j]/(M1[i]/ng) - 1.0;
				grid[ind2 + ind + j] = grid[ind2 + ind + j]/(M2[i]/ng) - 1.0;
			}
		}
	}
			...
			if(ntype == 1){
				...
				for(j=0;j<np;j++){
					...
					for(i=0;i<3;i++)
						post[i] = cysumf(post[i], -Lb/2.0, L);
					ind = ((size_t) ntype)*ng;
					M2_private[type[j]] += Density_Grid(&grid[((size_t) type[j])*ng + ind], nd, L, post, mass[j], window, R, R_times);
				}			
			}
