...
def Compute_Density_Grid(
    ...
    grid = exshalos.spectrum.spectrum.grid_compute(
        pos,
        vel,
        mass,
        np.int32(nmass),
...
def Compute_Power_Spectrum(
    ...
    x = exshalos.spectrum.spectrum.power_compute(
        grid,
        np.int32(ntypes),
        np.int32(nd),
        L,
...
def Compute_Power_Spectrum_individual(
    ...
    x = exshalos.spectrum.spectrum.power_compute_individual(
        grid,
        pos,
        np.int32(ntypes),
        np.int32(nd),
...
def Compute_BiSpectrum(
    ...
    x = exshalos.spectrum.spectrum.bi_compute(
        grid,
        np.int32(ntypes),
        np.int32(nd),
        L,
...
def Compute_TriSpectrum(
    ...
    x = exshalos.spectrum.spectrum.tri_compute(
        grid,
        np.int32(ntypes),
        np.int32(nd),
        L,
...
def Compute_Abundance(
    ...
    x = exshalos.spectrum.spectrum.abundance_compute(
        Mh,
        Mmin,
        Mmax,
        np.int32(Nm),
...
def Compute_Bias_Jens(
    ...
    if Lambda > 0.0:
        delta = exshalos.utils.Smooth_Fields(
            np.copy(grid),
            Lc=Lc,
            k_smooth=Lambda,
            Input_k=False,
    ...
    x = exshalos.spectrum.spectrum.histogram_compute(
        delta,
        Mh,
        np.int64(flag),
        Mmin,
...
def Compute_Voronoi(
    ...
    nd: Number of cells per dimension | int
    L: Size of the box in Mpc/h | float
    Om0: Omega matter at z=0 | float
    verbose: Output or do not output information in the c code | boolean
    nthreads: Number of threads used by openmp | int

    return: Quantities computed from the voronoi tesselation of the particles
    """

    precision = exshalos.finder.finder.finder_check_precision()
    ...
    voronoi = exshalos.finder.finder.voronoi_compute(
        pos,
        np.int32(nd),
        np.int32(nd),
        np.int32(nd),
