...
#Parameters of the ellipsoidal barrier
...
Nmin = 1        #Minimum number of cells/particles per halo
...
OUT_LPT = False #Set to True to output the positions (and velocities if OUT_VEL = True) of the particles
...
OUT_FLAG = False#Set to True to output the flag with the host halo of each particle

#Open the linear power spectrum (it must be at the redshift z and in units WITH h)
k, P = np.loadtxt("MDPL2_z00_matterpower.dat", unpack = True)

###Run ExSHalos###
#The full output is: halo positions, halo velocities , halo masses , particle positions , particle velocities , particle flags, and density grid
start = time.time()
x = exshalos.mock.Generate_Halos_Box_from_Pk(k, P, R_max = R_max, nd = Nd, Lc = Lc, Om0 = Om0, z = z, k_smooth = k_smooth, Nmin = Nmin, a = a, beta = beta, alpha = alpha, seed = seed, OUT_DEN = OUT_DEN, OUT_LPT = OUT_LPT, OUT_VEL = OUT_VEL, DO_2LPT = DO_2LPT, OUT_FLAG = OUT_FLAG, nthreads = nthreads)
end = time.time()
...
#Split the halos into multi tracers. It is just a simple example that split them using their masses
if(Ntracers > 1):
    types = np.zeros(nh)
...
if(OUT_VEL == False):
    grid = exshalos.simulation.Compute_Density_Grid(x["posh"], vel = None, mass = x["Mh"], type = types, nd = Nd, L = L, direction = None, window = window, R = R, R_times = R_times, interlacing = interlacing, nthreads = nthreads)
else:
    grid = exshalos.simulation.Compute_Density_Grid(x["posh"], vel = x["velh"], mass = x["Mh"], type = types, nd = Nd, L = L, direction = direction, window = window, R = R, R_times = R_times, interlacing = interlacing, nthreads = nthreads)

#Compute the power spectra
xh = exshalos.simulation.Compute_Power_Spectrum(grid, L = L, window = window, R = R, Nk = Nk, k_min = kmin, k_max = kmax, l_max = l_max, nthreads = 1, ntype = Ntracers)
end = time.time()
