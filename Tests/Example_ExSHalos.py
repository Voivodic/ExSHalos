...
# Parameter a of the ellipsoidal barrier (a = 1 for spherical collapse)
a = 1.0
# Parameter beta of the ellipsoidal barrier (beta = 0 for spherical collapse)
beta = 0.0
# Parameter alpha of the ellipsoidal barrier (alpha = 1 for spherical collapse)
alpha = 0.0

# Some options of ExSHalos
# Typical size of your box/survey in Mpc/h. Used to add super sample covariance
R_max = 10000.0
...
# Set to True to output the positions (and velocities if OUT_VEL = True) of the particles
OUT_LPT = True
OUT_VEL = False  # Set to True to output the velocities of the halos
# Set to True to use the second order LPT to compute the displacements and velocities (more precise but more time and memory)
DO_2LPT = False
...
# Change the second number by the redshift of your linear power spectrum
P = fDz(z) / fDz(0.0) * P
print("Running exshalos")
### Run ExSHalos###
...
start = time.time()
x = exshalos.mock.Generate_Halos_Box_from_Pk(
    k,
    P,
    R_max=R_max,
    nd=Nd,
...
# Radius of the sphere used to smooth the particles in Mpc/h (only used if window = EXPONENTIAL or Spherical)
R = 4.0
# Radius (in units of R) where particles are considered to construct the densidy grid (only used if window = EXPONENTIAL)
R_times = 5
...
# Number of cells in each direction (we can use a different number of cells to compute the spectrum)
Nd = 256
...
if OUT_VEL == False:
    grid = exshalos.simulation.Compute_Density_Grid(
        x["posh"],
        vel=None,
        mass=None,
        types=types,
...
xh = exshalos.simulation.Compute_Power_Spectrum(
    grid,
    L=L,
    window=window,
    R=R,
...
# Compute the density grid
if OUT_VEL == False:
    gridp = exshalos.simulation.Compute_Density_Grid(
        x["pos"],
        vel=None,
        mass=None,
        nd=Nd,
...
# Compute the power spectra
xp = exshalos.simulation.Compute_Power_Spectrum(
    gridp,
    L=L,
    window=window,
    R=R,
